...
/**
 * Async resolver that consults Firestore (admin source of truth) before
 * falling back to env-var-defined stations. Use this from request handlers
 * so newly-added stations from the admin app work without a redeploy.
 */
export async function resolveStationConfigByCode(
  code: string,
): Promise<StationConfig | null> {
  // Lazy import to avoid pulling firebase-admin into modules that don't need it.
  const { resolveStationImeiByCode } = await import(
    "@/lib/server/station-registry"
  );
  const resolved = await resolveStationImeiByCode(code);
  if (!resolved) {
    return getStationConfigByCode(code);
  }
  return {
