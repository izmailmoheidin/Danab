...
import {
  autoReturnPresentBatteries,
  getActiveRentedBatteryIds,
...
export async function getAvailableBattery(imei: string) {
  ...
  // Self-heal stale rental records: any battery physically present here
  // means the customer has returned it. Close those rentals BEFORE we
  // compute the "rented" set, so freshly-returned batteries become
  // available immediately for the next paying customer.
  await autoReturnPresentBatteries(imei, batteryIds);
