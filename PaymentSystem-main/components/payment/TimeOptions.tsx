...
const DEFAULT_STATION_NAME = "Danab Power Bank";
...
export function TimeOptions({
  ...
  // Resolve the station name only after mount. getStationName() reads
  // window.location, which differs between SSR and the browser and would
  // otherwise cause a hydration mismatch.
  const [stationName, setStationName] = useState(DEFAULT_STATION_NAME);
  useEffect(() => {
    setStationName(getStationName());
  }, []);
